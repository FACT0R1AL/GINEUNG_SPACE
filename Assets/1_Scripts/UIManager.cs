...
public class UIManager : MonoBehaviour
{
	...
	private void Update()
	{
		...
		// 예외 처리 (시간이 음수가 되거나 무한대 되는 것을 방지)
		...
		// TimeSpan 구조체를 이용해 분:초:밀리초 형식으로 변환
		System.TimeSpan timeSpan = System.TimeSpan.FromSeconds(remainingTime);

		// UI 텍스트에 반영 (60초가 넘으면 분단위, 60분이 넘으면 시간단위 등의 고려 없음)
	...
	{
		remainingDistanceText.text = $"{Random.Range(0, Mathf.RoundToInt(spaceshipPathLength * 1000f)).ToString("#,##0")}km 남음";
